...
from aifren.memory_v2_store.truth_scope_contract import normalize_truth_scope_label
...
def _activity_set(value: str, start: int, end: int) -> ActiveStateProposal | None:
    ...
    from aifren.memory_v2_store.active_state_contract import validate_active_state_value
    try:
...
def _character_profile_available(
    ...
    try:
        from aifren.character.character_scene_profile import cached_character_scene_profile
...
def admit_current_continuity_context(
    ...
    *,
    ...
    if re.search(r"\b(?:wear|wearing|wore|outfit|clothes|usual|normal\s+clothes)\b", query_text, re.I):
        try:
            from aifren.character.character_scene_profile import effective_profile_worn_items, profile_context_payload
