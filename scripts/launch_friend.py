...
def prepare_packaged_runtime():
    ...
    if sys.platform == "win32" and not _native_dll_directories:
        ...
        for directory in (site / "torch/lib", site / "llama_cpp/lib", site / "sklearn/.libs", site / "nvidia/cuda_runtime/bin"):
            if directory.is_dir():
...
def package_environment(layout, data_root=None):
    ...
    if profile == "nvidia-stock-v1":
        ...
        environment["PATH"] = os.pathsep.join([str(site / "torch/lib"), str(site / "llama_cpp/lib"), str(site / "sklearn/.libs"),
