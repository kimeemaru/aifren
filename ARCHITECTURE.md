...
current/history meaning. Before/after ordering uses canonical sources.

Explicit favorite-color assertions preserve bounded literal labels (including
uncommon and multiword values), rather than requiring a color dictionary. This
does not infer physical scene colors or broaden legacy imported-text authority.
Current and historical admission share the label boundary; corrections retain
their original source and ordering. Previously missed records are not silently
recurated or regenerated by this change.

Historical projection stays at most two passages with 220 total source characters. Immediate
