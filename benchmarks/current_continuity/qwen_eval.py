...
def _cases() -> tuple[BehavioralCase, ...]:
    ...
    scenario = _scope("scenario", "Gensokyo")
    activity_sleep = (
    ...
    def unrelated_check(response: str) -> tuple[bool, str]:
        passed = not _contains_any(response, ("noita", "gpu", "roleplay", "gensokyo"))
    ...
    def residence_after_exit_check(response: str) -> tuple[bool, str]:
        passed = not _contains_any(response, ("scarlet mansion", "gensokyo")) and _contains_any(response, (
    ...
    return (
        ...
        BehavioralCase("scenario_exit_isolation", "enter RP -> scenario fact -> exit -> same question",
            _context(_message("user", "Let's roleplay that we're in Gensokyo."),
