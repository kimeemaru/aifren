...
class IdentityNameAdmissionExperimentTests(unittest.TestCase):
    def setUp(self):
        ...
        self.repository.ensure_character(self.character, "A")
        scope = self.repository.active_truth_scope(self.character)
        provenance = {"kind": scope.kind, "scope_id": scope.truth_scope_id}
        message = {
            "role": "user", "content": "My name is Elena.",
            "timestamp": "2020-01-01T00:00:00Z", "truth_scope": provenance,
        }
        self.conversation.write_text(json.dumps([message, {
    ...
    def test_other_character_name_never_enters_primary_admission_candidates(self):
        ...
        try:
            other_repository = MemoryV2Repository(other_writer.store)
            other_repository.ensure_character(self.other_character, "B")
            other_scope = other_repository.active_truth_scope(self.other_character)
            provenance = {"kind": other_scope.kind, "scope_id": other_scope.truth_scope_id}
            other_message = {
                "role": "user", "content": "My name is Blair.",
                "timestamp": "2020-01-01T00:00:00Z", "truth_scope": provenance,
            }
            other_conversation.write_text(json.dumps([other_message, {
                "role": "assistant", "content": "Synthetic completed response.",
                "timestamp": "2020-01-01T00:00:01Z", "truth_scope": provenance,
            }]), encoding="utf-8")
            other_writer.observe_canonical_user_message(
                other_message, conversation_index=0, conversation_file=other_conversation,
            )
            report = evaluate_identity_name_admission(self.repository, self.character)
            self.assertTrue(all(trace.candidate_claim_ids == (self.created["claim_id"],) for trace in report.traces))
