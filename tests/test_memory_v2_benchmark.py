...
class MemoryV2BenchmarkTests(unittest.TestCase):
    ...
    def test_core_fixture_covers_non_factual_and_temporal_source_cases(self):
        ...
        self.assertEqual(
            next(claim for claim in self.fixture.claims if claim.claim_id == "lyra-tea-green").source_event_ids,
            ("lyra-008", "lyra-013"),
        )
    ...
    def test_reference_adapter_validates_gold_retrieval_contract(self):
        ...
        self.assertEqual(retrieved["allergy-needle"], ["lyra-walnut-allergy"])
    ...
    def test_consolidation_metrics_detect_unsupported_and_bad_supersession(self):
        ...
        invalid = valid + [ClaimProposal("unsupported", "lyra", ("not-an-event",))]
    ...
    def test_retrieval_query_excludes_assistant_turns_and_is_bounded(self):
        query = RetrievalQuery("lyra", "What tea do I prefer?", "2026-08-01T00:00:00Z", recent_user_turns=("I had tea yesterday.",))
        self.assertEqual(query.recent_user_turns, ("I had tea yesterday.",))
        with self.assertRaises(ValueError):
            RetrievalQuery("lyra", "x", "2026-08-01T00:00:00Z", recent_user_turns=("x",) * 9)
    ...
    def test_trace_metrics_detect_missing_trace_and_stale_embedding_use(self):
        ...
        retrieved = {"trace-abstention": [], "stale-embedding": ["lyra-cobalt-needle"]}
        outcomes = {
            "trace-abstention": RetrievalOutcome((), (), "no_relevant_candidate"),
            "stale-embedding": RetrievalOutcome(("lyra-cobalt-needle",), (RetrievalTrace("lyra-cobalt-needle", ("semantic",), "semantic_score"),)),
        }
    ...
    def test_trace_metrics_require_channel_and_reason_for_selected_claims(self):
        ...
        report = evaluate_retrieval(
            fixture, {"duplicate-multi-channel": ["lyra-pokemon-episode"]},
            outcomes_by_case={"duplicate-multi-channel": RetrievalOutcome(("lyra-pokemon-episode",), (RetrievalTrace("lyra-pokemon-episode", ("fts",)),))},
        )
