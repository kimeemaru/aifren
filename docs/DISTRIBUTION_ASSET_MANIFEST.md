...
referenced by the current Unity client. It records evidence presently in the
private repository; it does not grant rights that the repository does not
...
| `Assets/Resources/LocalCharacter/model.vrm` and `.meta` (currently ignored in private checkout) | Current default avatar resource selected by `CharacterAvatarConfig.json`: VRoidPreset_A | Official [VRoidPreset_A - Z conditions](https://vroid.pixiv.help/hc/en-us/articles/4402394424089-VRoidPreset-A-Z); public-root construction must copy this exact default asset and meta file | VRoidPreset_A - Z conditions of use, not `LICENSE.md` | No attribution required; alteration and redistribution are allowed subject to the conditions | Yes, include in the clean public root | Yes, as part of the application subject to the conditions | `SAFE_BOTH`; do not distribute under CC0 or sell the raw/unmodified sample VRM for a fee |
