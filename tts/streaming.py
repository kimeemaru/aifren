...
It deliberately contains no provider logic: Kokoro and future local
...
class TtsSynthesisResourceManager:
    ...
    def wait_for_provider_idle(self, timeout: float = 30.0) -> bool:
        ...
        while self.provider_active() and time.monotonic() < deadline:
    ...
    def _fallback_to_cpu(self, unit_index: int, category: str, direct: bool) -> bool:
        ...
        if not callable(fallback) or self._cancelled.is_set():
            return False
        ...
        return switched and not self._cancelled.is_set()
    ...
    def prepare(self, text: str, *, unit_index: int = 0, direct: bool = False) -> SynthesisRecoveryResult:
        ...
        for attempt in (1, 2, 3):
            ...
            try:
                ...
                cancelled_now = self._cancelled.is_set()
                development_flight_recorder().mark(
...
class StreamingSpeechQueue:
    ...
    def submit(self, text: str, *, presentation_text: str | None = None) -> bool:
        ...
        with self._overflow_lock:
            if self._overflow_item is not None:
                ...
                self._overflow_item = (
                    old_index,
                    self._join_units(old_spoken, spoken),
                    self._join_units(old_presentation, presentation),
                    old_at,
                    False,
                )
                coalesced_index = old_index
            else:
                coalesced_index = None
        if coalesced_index is not None:
            development_flight_recorder().mark(
                "tts_synthesis_coalesced", chunk_index=int(coalesced_index),
        ...
        try:
            ...
            development_flight_recorder().mark(
                "tts_synthesis_queued", chunk_index=chunk_index,
                queue_depth=self._queue.qsize(), pending_jobs=self.pending_jobs,
                characters=len(spoken), words=len(spoken.split()),
            )
            return True
        except Full:
            # Keep one bounded overflow job and coalesce only complete sentences
            # which have not begun synthesis. No spoken content is dropped and
            # the provider still has at most one synthesis call in flight.
            with self._overflow_lock:
                if self._overflow_item is None:
                    self._overflow_item = item[:-1] + (False,)
                    self._next_chunk_index += 1
                    coalesced_index = chunk_index
                else:
                    old_index, old_spoken, old_presentation, old_at, _ = self._overflow_item
                    self._overflow_item = (
                        old_index,
    ...
    def cancel(self) -> None:
        ...
        try:
            ...
            pass
        try:
            self._prepared.put_nowait(self._STOP)
        except Full:
            pass
    ...
    def _put_prepared(self, item: object) -> bool:
        while not self._cancelled.is_set():
            try:
                self._prepared.put(item, timeout=0.10)
